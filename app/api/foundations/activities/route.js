...
// Database triggers synchronize linked Applications activities in the same transaction.
...
export async function DELETE(req) {
  try {
    ...
    if (!body.id) return Response.json({ error: "id required" }, { status: 400 });
    const supabase = getAdminClient();
    // Deleting the source and its linked mirror is atomic in the database.
