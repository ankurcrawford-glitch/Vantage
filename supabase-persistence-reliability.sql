...
begin
  ...
  select * into v from public.essay_versions where essay_id=e.id order by is_current desc, version_number desc limit 1;
...
alter table public.essay_permissions add column if not exists role text not null default 'reviewer';
...
begin
  ...
  insert into public.essay_permissions(essay_id,user_id,role,commenter_name)
    values(inv.essay_id,auth.uid(),inv.role,coalesce(inv.invitee_name,recipient))
    on conflict(essay_id,user_id) do update set role=excluded.role,commenter_name=excluded.commenter_name;
  update public.essay_invitations set status='accepted' where id=inv.id;
  return inv.essay_id;
end $$;
...
returns boolean language sql stable security definer set search_path=public as $$
  select auth.uid() is not null and exists(
    ...
    where ep.essay_id=p_essay_id and ep.user_id=auth.uid() and u.email_confirmed_at is not null
      and i.status='accepted' and (i.expires_at is null or i.expires_at>=now())
  );
$$;
